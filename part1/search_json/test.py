...
class SettingsTestCase(SkyproTestCase, ResponseTestsMixin):
    ...
    def test_tours_with_search_is_available_and_works_correct(self):
        url = '/tours?search=лу'
        test_options = {
            "url": url,
            "method": 'GET',
            ...
            "student_response": self.app.get(
                url, json=""),
            "expected": list,
            "answer": [{ "title": "Лучшие панорамы вечернего города", "price_rur": 1800, "category": "walk"}]
        }
        response = self.check_status_code_jsonify_and_expected(**test_options)
        self.assertTrue(
            response.is_json,
    ...
    def test_tours_with_no_result_returns_all(self):
        url = '/tours?search=qwe'
        test_options = {
            "url": url,
            "method": 'GET',
            "code": [200],
